...
p1 <- ggplot(dat, aes(x = date, y = active, group = Clusters, color = Clusters)) + geom_smooth(method = 'loess',formula='y ~ x') + scale_x_date(date_labels = "%b %d") + theme_classic() + ylab("Date") + xlab("Active")
...
eff <- ggeffect(mod1, terms = c("workplace_closingF", "gatherings_restrictionsF", "stay_home_restrictionsF", "transport_closingF"))
div(plot(eff), align = "center")
```

```{r, echo=FALSE, warning = FALSE, comment = FALSE, silent = T}
eff <- ggeffect(mod1, terms = c("testing_policyF", "contact_tracingF"))
div(plot(eff), align = "center")
```
...
div(subplot(ggplotly(p1, tooltip = c("colour", "y")) %>% layout(showlegend = FALSE, title = ""),
 ggplotly(p2, tooltip = c("colour", "y"))%>% layout(showlegend = FALSE,title = ""), 
ggplotly(p3, tooltip = c("colour", "y")) %>% layout(showlegend = FALSE, title = "Gatherings           Testing            Tracing"), shareY = T
), align = "center") 
```
