...
Therefore, some variables as the size of population are considered from the [World Bank Open Data](https://data.worldbank.org/) to have some additional covariates that can influence the variation in government responses to COVID-19. 
...
```{r, echo = FALSE, comment = F, warning= F, silent = T}
p2 <- dat %>% ggplot(aes(x=as.integer(date2), y=E1_Income_support, group=Clusters, color=Clusters)) + geom_smooth(method = 'loess',formula='y ~ x')  + ylab("Income Support") + xlab("Days after first case")+ xlim(0,max(as.integer(dat$date2))) + theme_classic() 
...
```{r, echo = FALSE, comment = F, warning= F, silent = T}
p3 <- dat %>% ggplot(aes(x=as.integer(date2), y=E2_Debt_contract_relief, group=Clusters, color=Clusters)) + geom_smooth(method = 'loess',formula='y ~ x')  + ylab("Debt Contract Relief") + xlab("Days after first case")+ xlim(0,max(as.integer(dat$date2))) + theme_classic()

a <- ggplotly(p3, tooltip = c("id", "E1_Income_support"))
...
a$x$data[[4]]$name <- c(paste0(Cl4, collapse = " "))
a$x$data[[5]]$name <- c(paste0(Cl5, collapse = " "))
p3 <- a %>% layout(legend = list(orientation = "h", y =-.2))
...
```{r, echo = FALSE, comment = F, warning= F, silent = T}
p1 <- dat %>% ggplot(aes(x=as.integer(date2), y=H4_Emergency_investment_in_healthcare, group=Clusters, color=Clusters))  +
  geom_smooth(method = 'loess',formula='y ~ x') + scale_x_date(date_labels = "%b %d") + ylab("Emergency investment in healthcare") + xlab("Days after first case")+ xlim(0,max(as.integer(dat$date2))) + theme_classic()
p2 <- dat %>% ggplot(aes(x=as.integer(date2), y=H5_Investment_in_vaccines, group=Clusters, color=Clusters))  +  geom_smooth(method = 'loess',formula='y ~ x') + scale_x_date(date_labels = "%b %d") + ylab("Investment in vaccines") + xlab("Days after first case")+ xlim(0,max(as.integer(dat$date2))) + theme_classic()
...
```{r, echo = FALSE, comment = F, warning= F, silent = T}
p1 <- dat %>% ggplot(aes(x=as.integer(date2), y=pca_hs, group=Clusters, color=Clusters))  +
  geom_smooth(method = 'loess',formula='y ~ x') + scale_x_date(date_labels = "%b %d") + ylab("Health Investment variable (PCA)") + xlab("Days after first case")+ xlim(0,max(as.integer(dat$date2))) + theme_classic() 
