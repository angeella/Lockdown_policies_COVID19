...
outSilvia <- readRDS(paste0(path, "Presentation/Data/dpca.rds"))
```
...
ggplot(outSilvia, aes(x=PC, y=Loadings, fill=policies, group=position))+ 
 geom_bar(stat="identity", position="dodge")+
  coord_flip()+scale_fill_manual("", breaks=c(apol, rpol), 
                                 values=c(peach[1], peach[2],                                           peach[3], gray[1:8]),
                                 labels  = Hmisc::capitalize(gsub("_", " ", policies))
                                    )
```
...
a <- ggplot(gd, aes(x = as.integer(date2), y = active_over_pop_1000, color = Clusters)) +
  ...
  theme_classic()  + ylab("Active/pop per 1000") + xlab("Day at first contagion")
...
```{r, echo=FALSE, warning = FALSE, comment = FALSE, silent = T, fig.height=4}
path <- "~/GitHub/Lockdown_policies_COVID19/" #Insert your path
load(paste0(path, "Presentation/Data/plot_mod.RData"))

a <- ggplotly(p1, tooltip = c("colour", "y")) %>% layout(showlegend = FALSE, title = "")

a$x$data[[1]]$marker$color <- a$x$data[[1]]$marker$line$color
a$x$data[[2]]$marker$color <- a$x$data[[2]]$marker$line$color
a$x$data[[3]]$marker$color <- a$x$data[[3]]$marker$line$color
a$x$data[[4]]$marker$color <- a$x$data[[4]]$marker$line$color
a$x$data[[5]]$marker$color <- a$x$data[[5]]$marker$line$color

b <-  ggplotly(p2, tooltip = c("colour", "y"))%>% layout(showlegend = FALSE,title = "")

b$x$data[[1]]$marker$color <- b$x$data[[1]]$marker$line$color
b$x$data[[2]]$marker$color <- b$x$data[[2]]$marker$line$color
b$x$data[[3]]$marker$color <- b$x$data[[3]]$marker$line$color
b$x$data[[4]]$marker$color <- b$x$data[[4]]$marker$line$color

cc <-ggplotly(p3, tooltip = c("colour", "y")) %>% layout(showlegend = FALSE, title = "Gatherings Restrictions           Testing            Tracing")

cc$x$data[[1]]$marker$color <- cc$x$data[[1]]$marker$line$color
cc$x$data[[2]]$marker$color <- cc$x$data[[2]]$marker$line$color
cc$x$data[[3]]$marker$color <- cc$x$data[[3]]$marker$line$color

subplot(a, b, cc, shareY = T) 
```
