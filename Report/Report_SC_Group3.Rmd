...
As regards contagion data, we used as main variable the active cases, defined as the total confirmed cases (cumulative confirmed positive cases including presumptive positive) minus total deaths minus total recovered. For further information see @Jhon. 
...
tab_EC <- data.frame(Name = c("Income Support", "Debt/contract relief for households", "Fiscal measures", "International support"), Measurement = c("Ordinal", "Ordinal", "USD", "USD"), Description = c("Government income support to people that lose their jobs", "Government policies imposed to freeze financial obligations", "Economic fiscal stimuli", "monetary value spending to other countries"), Category = c("0 - no income support; 1 - government is replacing less than 50% of lost salary, 2 - government is replacing more than 50% of lost salary", "0 - No; 1 - Narrow relief, specific to one kind of contract; 2 - broad debt/contract relief", "", ""))
knitr::kable(tab_EC)
```
...
```{r figs2, fig.align='center', echo=FALSE, fig.cap="\\label{fig:figs2}Clusters map.", eval = F}
...
<!-- Some countries have underestimated the dangerousness of the Coronavirus disease 2019 (COVID-19) and the importance of applying the containment measures. The little concern of some countries regarding the COVID-19 infectious disease is due to many and different reasons. Some countries decided to save the economy instead of people's lives as a method of responding to war; in this case, a pandemic war. Thanks to the @Oxford data sets, we know which actions each government took and when. The indicators of government response considered are $17$ in total, which can be resumed in indicators of a lockdown/social distancing measures, contact tracing measures, movement restrictions, testing policy, public health measures, and governance and socio-economic measures.--> 
...
The dependent variable is the number of active persons; therefore, a count data model is considered. To control the overdispersion of our data, the negative binomial regression with Gaussian-distributed random effects is performed using the \texttt{glmmTMB} R package developed by @glmmTMB. Let $n$ countries, and country $i$ is measured at $n_i$ time points $t_{ij}$. The active person $y_{ij}$ count at time $t+14$, where $i=1,\dots, n$ and $j = 1, \dots,n_i$, follows the negative binomial distribution:
